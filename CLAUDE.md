...
**Avatar custom emoji** (opt-in, `AVATAR_EMOJI_ENABLED`, default off): when sharing a **profile or a post**, the bot can show the avatar (the profile's, or the post author's) as an inline **custom emoji** prepended to the message. `avatarEmojiService` (`src/services/avatarEmoji.ts`) downloads the avatar, resizes it to a 100×100 PNG (`src/utils/image.ts`, via `jimp`), and uploads it as a `custom_emoji` sticker — pooling avatars into bot-owned sets (200 max each) tracked in `emoji_sticker_sets`, with one emoji per X user cached in `avatar_emoji` (reused while the avatar is unchanged, replaced in place when it changes). The avatar source is `ProfileData.avatarUrl` for profiles and `TweetData.authorAvatarUrl` for posts (populated by the providers from the same tweet response — no extra fetch; left null by providers that don't expose it). The service is **best-effort**: any failure (download, image, Telegram) resolves to `null` and the message is sent without the emoji. The handlers (`private.ts` `replyWithPost`, `inline.ts` `safeEditRich`) inject the emoji into the Rich Message via `withAvatarEmoji` (`src/formatters/richMessage.ts`) and **retry the send without it** if Telegram rejects the custom emoji. The service is built once in `buildBot` whenever an owner id can be resolved (`AVATAR_EMOJI_OWNER_ID`, else the first admin — sets are attributed to that user) and attached to `ctx.services.avatarEmoji`. Enablement is a **runtime toggle** in `RuntimeConfig` (`avatarEmojiEnabled`): `AVATAR_EMOJI_ENABLED` sets the initial state and admins flip it with **`/avataremoji on|off`** (the handlers gate on the runtime flag, so no restart needed). **Telegram only delivers bot-sent custom emoji when the bot is eligible** — the owner has Telegram Premium, or the bot has a Fragment username; otherwise viewers see the fallback glyph.
...
- `avatar_emoji` — one row per X user (unique `username`) → the `custom_emoji_id` made from their avatar, plus the current `avatar_url`, the sticker's `sticker_file_id`, and `set_name`. A user always maps to one emoji: an unchanged avatar reuses it, and a changed avatar replaces the sticker in place (delete + re-add in the same set) so the pool doesn't fill with stale emoji. Written via a non-transactional DB handle so the mapping survives even if the request transaction rolls back.
